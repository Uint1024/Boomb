/* Generated by Nim Compiler v0.11.2 */
/*   (c) 2015 Andreas Rumpf */
/* The generated code is subject to the original license. */
/* Compiled for: Windows, i386, gcc */
/* Command for C compiler:
   gcc.exe -c  -w  -IC:\Nim\lib -o c:\users\john\documents\nim\bomberman\nimcache\stdlib_winlean.o c:\users\john\documents\nim\bomberman\nimcache\stdlib_winlean.c */
#define NIM_INTBITS 32
#include "nimbase.h"

#include <string.h>
typedef struct Tguid106614 Tguid106614;
typedef struct TGenericSeq TGenericSeq;
typedef struct NimStringDesc NimStringDesc;
typedef struct Tfiletime104245 Tfiletime104245;
typedef NI8 TY106627[8];
struct  Tguid106614  {
NI32 D1;
NI16 D2;
NI16 D3;
TY106627 D4;
};
typedef NI8 TY106708[8];
struct  TGenericSeq  {
NI len;
NI reserved;
};
struct  NimStringDesc  {
  TGenericSeq Sup;
NIM_CHAR data[SEQ_DECL_SIZE];
};
typedef N_STDCALL_PTR(void, TY105625) (Tfiletime104245* lpsystemtimeasfiletime);
struct  Tfiletime104245  {
NI32 dwlowdatetime;
NI32 dwhighdatetime;
};
N_NIMCALL(void*, nimLoadLibrary)(NimStringDesc* path);
N_NOINLINE(void, nimLoadLibraryError)(NimStringDesc* path);
N_NIMCALL(void*, nimGetProcAddr)(void* lib, NCSTRING name);
NIM_CONST TY106708 TMP149 = {((NI8) -114),
((NI8) -23),
((NI8) 118),
((NI8) -27),
((NI8) -116),
((NI8) 116),
((NI8) 6),
((NI8) 62)}
;
NIM_CONST TY106708 TMP150 = {((NI8) -107),
((NI8) -54),
((NI8) 0),
((NI8) -128),
((NI8) 95),
((NI8) 72),
((NI8) -95),
((NI8) -110)}
;
STRING_LITERAL(TMP161, "kernel32", 8);
STRING_LITERAL(TMP162, "kernel32", 8);
Tguid106614 Wsaidconnectex_106710;
Tguid106614 Wsaidacceptex_106714;
Tguid106614 Wsaidgetacceptexsockaddrs_106718;
static void* TMP160;
TY105625 Dl_105624;

N_NIMCALL(NI64, rdfiletime_105604)(Tfiletime104245 f) {
	NI64 result;
	result = 0;
	result = (NI64)(((NI64)(NU64)(NU32)(f.dwlowdatetime)) | (NI64)((NU64)(((NI64)(NU64)(NU32)(f.dwhighdatetime))) << (NU64)(IL64(32))));
	return result;
}
NIM_EXTERNC N_NOINLINE(void, stdlib_winleanInit)(void) {
	Tguid106614 LOC1;
	Tguid106614 LOC2;
	Tguid106614 LOC3;
	memset((void*)(&LOC1), 0, sizeof(LOC1));
	LOC1.D1 = ((NI32) 631375801);
	LOC1.D2 = ((NI16) -8717);
	LOC1.D3 = ((NI16) 18016);
	memcpy((void*)LOC1.D4, (NIM_CONST void*)TMP149, sizeof(LOC1.D4));
	Wsaidconnectex_106710 = LOC1;
	memset((void*)(&LOC2), 0, sizeof(LOC2));
	LOC2.D1 = ((NI32) -1254720015);
	LOC2.D2 = ((NI16) -13396);
	LOC2.D3 = ((NI16) 4559);
	memcpy((void*)LOC2.D4, (NIM_CONST void*)TMP150, sizeof(LOC2.D4));
	Wsaidacceptex_106714 = LOC2;
	memset((void*)(&LOC3), 0, sizeof(LOC3));
	LOC3.D1 = ((NI32) -1254720014);
	LOC3.D2 = ((NI16) -13396);
	LOC3.D3 = ((NI16) 4559);
	memcpy((void*)LOC3.D4, (NIM_CONST void*)TMP150, sizeof(LOC3.D4));
	Wsaidgetacceptexsockaddrs_106718 = LOC3;
}

NIM_EXTERNC N_NOINLINE(void, stdlib_winleanDatInit)(void) {
if (!((TMP160 = nimLoadLibrary((NimStringDesc*) &TMP161))
)) nimLoadLibraryError((NimStringDesc*) &TMP162);
	Dl_105624 = (TY105625) nimGetProcAddr(TMP160, "GetSystemTimeAsFileTime");
}

